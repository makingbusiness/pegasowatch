...
(function ($) {
  ...
  $(".header-dropdown.user-menu > a").on('click', function(e) {
    e.preventDefault();
  });

  // Simula click dropdown menu mobile
  $("#productosJS").on('click', function() {
    $(this).children("span").trigger( "click" );
  });

  //Explore section Style
  $(".explore-section .container").css({
    'padding-top': ($(".explore-section").height() - $(".explore-section .container").height()) / 2 + "px"
  });

  $("ul.list li a.nav-link").on("click", function() {
    $(this).parent("li").addClass("active").siblings("li").removeClass("active");
  });

  // Funcion para el cambio de color de un producto
  $(".product-details .config-swatch-list a").on('click', function(event) {
    event.preventDefault();
    $(this).parent("li").siblings().removeClass("active");
  });

  $(".table-invoice > tbody > tr > td:last-child").on("click", function() {
    $("#invoices-tab").trigger("click");
  });
